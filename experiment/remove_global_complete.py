...
from greedy.global_remove import global_remove_list
...
def store(top_k, type, id, k, m, name, number):
    root = os.path.abspath(os.path.dirname(os.getcwd()))+"/ESBasedonSimilarity/"
    folder_name = "global_reomove_complete_k_" + str(k) + "_m_" + str(m)
    folder_path = os.path.join(root, "global_res", folder_name, name)
    ...
    if type == "top":
        res_path = os.path.join(folder_path, str(id),"{}_top{}.nt".format(id, number))
    if type == "rank":
        file_origin_list = []
...
def remove_list_complete(remove_list, name, k, m):
    """
    ...
    for file in file_path:
        ...
        count, res = global_remove_list(remove_list, t, key)
        if count >= 5:
            top_5 = res[:5]
        elif count < 5:
            top_10 = res[:count]
        top_5.sort()
        store(top_5, "top", value, k, m, name, 5)
        if count >= 10:
            top_10 = res[:10]
        elif count < 10:
            top_10 = res[:count]
        top_10.sort()
        store(top_10, "top", value, k, m, name, 10)
